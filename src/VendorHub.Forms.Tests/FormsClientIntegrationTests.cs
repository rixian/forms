...
public class FormsClientIntegrationTests
{
    ...
    public async System.Threading.Tasks.Task ListFormSubmissionsAsync()
    {
        ...
        ICollection<FormSubmissionSummary> submissions = await this.client.ListSubmissionsAsync(form.FormId, tenantId).ConfigureAwait(false);
        FormSubmissionSummary submissionInfo = submissions.FirstOrDefault(s => s.AttachmentCount > 0);
        FormSubmission submission = await this.client.GetSubmissionAsync(form.FormId, submissionInfo.SubmissionId, tenantId).ConfigureAwait(false);
    }
    ...
    public async System.Threading.Tasks.Task ListForms2Async()
    {
        try
        {
            ...
            submission = await this.client.GetSubmissionAsync(form.FormId, submission.SubmissionId, tenantId).ConfigureAwait(false);
            submission.Should().NotBeNull();
